...
CodePress = {
	...
	keyHandler : function(evt) {
		...
		if( CodePress.suggest(fromChar) == 1 ) { return false; }
	...
	syntaxHighlight : function(flag) {
		...
		console.log(arguments[1] + arguments[2] + ' ---- '+ x);
	...
	actions : {
		...
		suggest : function(text) {
			editor = CodePress.getEditor();
			tests = new Array('heelo', 'gooodbye');
			word = CodePress.getLastWord();
			reg = new RegExp("^"+word+text, 'gi');
			for (i=0; i< tests.length; i++) {
				if (tests[i].match(reg)) {
					//code = editor.innerHTML+text;
					//console.log('match: '+tests[i]+" - "+code);
					//code = code.replace(text, tests[i]);
					//console.log('match: '+tests[i]+" - "+text);
					//thing = document.getElementById('hey');
					//reg = new RegExp(wor, 'gi');
					//editor.innerHTML = editor.innerHTML.replace(reg,tests[i]);
					//editor.innerHTML = 'heheh';
					var pattern = new RegExp(word+cc,'i');
					CodePress.syntaxHighlight('snippets', pattern, tests[i]);
					//window.getSelection().getRangeAt(0).insertNode(document.createTextNode(code))
					return 1;
					//text = tests[i];
				}
			}
